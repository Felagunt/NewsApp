...
class DetailsViewModel @Inject constructor(private val repository: NewsRepository): ViewModel() {
    ...
    fun getSavedArticles() = viewModelScope.launch(Dispatchers.IO) {
        ...
        println("Favorites: ${res.size} ")
        //Toast.makeText( context,"Favorites: ${res.size} ", Toast.LENGTH_SHORT).show()
    ...
    fun removeFavoriteArticles(article: Article) = viewModelScope.launch(Dispatchers.IO) {
